...
def get_data_merged_train(bureau_transformed: pd.DataFrame,
                          previous_application_transformed: pd.DataFrame,
                          train_splits: dict[str, pd.DataFrame]) -> (
        pd.DataFrame):
    # Bureau Processing Pipeline
    bureau_pipeline = Pipeline([
        ('extract_bureau', FunctionTransformer(lambda X: X[0])),  # Access bureau_transformed
        ('add_counts', SKIDCounter(id_column='SK_ID_BUREAU', count_column_name='SK_ID_BUREAU_count')),
        ('drop_columns', ColumnDropper(columns_to_drop=['SK_ID_BUREAU'])),
        ('aggregate_by_key', AggregateByKey(key_column='SK_ID_CURR')),  # Aggregate by SK_ID_CURR
        ('add_counts_to_transformed', AddCountsToTransformed(count_column_name='SK_ID_BUREAU_count'))
    ])

    # Previous Application Processing Pipeline
    previous_application_pipeline = Pipeline([
        ('extract_previous', FunctionTransformer(lambda X: X[1])),
        ('add_counts', SKIDCounter(id_column='SK_ID_PREV', count_column_name='SK_ID_PREV_count')),
        ('drop_columns', ColumnDropper(columns_to_drop=['SK_ID_PREV'])),
        ('aggregate_by_key', AggregateByKey(key_column='SK_ID_CURR')),  # Aggregate by SK_ID_CURR
        ('add_counts_to_transformed', AddCountsToTransformed(count_column_name='SK_ID_PREV_count'))
    ])

    # Final Pipeline
    final_pipeline = Pipeline([
        # Handle multiple inputs using MultiInputWrapper
        ('multi_input', MultiInputWrapper()),
        # Merge bureau pipeline output into main table
        ('merge_bureau', MergePipelineOutputWithMainTable(
            pipeline=bureau_pipeline,
            main_table=train_splits['data_train']
        )),
        # Merge previous application pipeline output into main table
        ('merge_previous_application', MergePipelineOutputWithMainTable(
            pipeline=previous_application_pipeline,
            main_table=train_splits['data_train']
        ))
    ])
    ...
    res = final_pipeline.fit_transform(
        (bureau_transformed,
